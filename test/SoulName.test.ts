...
describe("Soul Name", () => {
  ...
  describe("mint", () => {
    it("should mint from owner", async () => {
      const mintTx = await soulName
        ...
        ](address1.address, SOUL_NAME1, YEAR, ARWEAVE_LINK1);
    ...
    it("should success to mint a name twice to the same idenity", async () => {
      ...
      await soulName
        ...
        [
          "mint(address,string,uint256,string)"
        ](address1.address, SOUL_NAME2, YEAR, ARWEAVE_LINK2);
    });
    ...
    it("should fail to mint duplicated name", async () => {
      ...
      await expect(
        soulName
          ...
          [
            "mint(address,string,uint256,string)"
          ](address1.address, SOUL_NAME1, YEAR, ARWEAVE_LINK2)
      ).to.be.rejected;
    });
    ...
    it("should fail to mint duplicated link", async () => {
      ...
      await expect(
        soulName
          ...
          [
            "mint(address,string,uint256,string)"
          ](address1.address, SOUL_NAME2, YEAR, ARWEAVE_LINK1)
      ).to.be.rejected;
    });
    ...
    it("should fail to mint invalid link", async () => {
      await expect(
        soulName
          ...
          [
            "mint(address,string,uint256,string)"
          ](address1.address, SOUL_NAME1, YEAR, ARWEAVE_LINK_INVALID)
      ).to.be.rejected;
    });
  ...
  describe("read data from the SoulName smart contract", () => {
    ...
    it("isAvailable false with an existing name - case insensitive", async () => {
      await expect(await soulName["exists(string)"](SOUL_NAME1.toLowerCase()))
        .to.be.true;
      await expect(await soulName["exists(string)"](SOUL_NAME1.toUpperCase()))
        .to.be.true;
      await expect(
  ...
  describe("expiration data", async () => {
    ...
    it("should allow mint same name if previous has expired", async () => {
      ...
      await soulName
        ...
        [
          "mint(address,string,uint256,string)"
        ](address2.address, SOUL_NAME1, YEAR, ARWEAVE_LINK2);
    });
